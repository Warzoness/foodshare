...
const useFcmToken = () => {
  ...
  const loadToken = async () => {
    ...
    // Tự động gửi token lên server để lưu
    if (token) {
  ...
  const saveTokenToServer = async (tokenToSave: string) => {
    try {
      ...
      if (data.success) {
        console.log("Token đã được lưu thành công trên server");
      } else {
        console.error("Lỗi khi lưu token:", data.error);
      }
    } catch (error) {
